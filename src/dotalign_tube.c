...
##########################################################################
$Id: dotalign_tube.c,v 1.2 2005/02/17 23:13:32 aheger Exp $
...
/* define safe: in dotalign_tube.c, ires goes out of bounds in line 353 for bestpercol */
#define SAVE 1
...
CALCTYPE __DotAlign_Tube ( row, col, 
			   score, ndots,
			   roll, 
			   minrow, maxrow, 
			   mincol, maxcol, 
...

{
    ...
    int *bestpercolstack = Malloc( (maxcol + 1) * sizeof (int ));
    int *bestpercol      = Malloc( (maxcol + 1) * sizeof (int ));
    int *topdot          = Malloc( (maxrow + 1) * sizeof (int ));
    CALCTYPE *m          = Malloc( ndots * sizeof(CALCTYPE));
  ...
  /* iterate through rows */
    for ( idot = dotp[minrow]; idot < ndots; idot++ ) { /* iterate through rows starting at firstrow */
	if (idot < 0) continue; 
	...
	diag = ires - jres;
	/*	if (diag < 0) diag = -diag; */
	if (diag < mindiag || diag > maxdiag) continue; /* only look at dots inside of tube */
...
#ifdef DEBUG
	printf("idot %5i; trace[idot] %5i; m[idot] %5.2f; best %5.2f; ires=%i\n",
	       idot, trace[idot], m[idot], best,ires);
...
void dotalign_tube (    row, col, score, ndots,
		     roll, 
		     minrow, maxrow, 
		     mincol, maxcol,
...
{ 
    ...
    int lastdot;
    int *trace = Malloc( ndots * sizeof(int));
    
#ifdef DEBUG
    printf("DotAlign_Tube called with mincol %i, maxcol %i , minrow %i, maxrow %i, roll %i, ndots %i\n",
	   mincol, maxcol, minrow, maxrow, roll, ndots) ;
    printf("gap parameters: iopen %5.2f, ielon %5.2f, jopen %5.2f, jelon %5.2f mindiag %i maxdiag%i\n",
	   iopen, ielon, jopen, jelon, mindiag, maxdiag);
    ...
    dotp = Malloc( (maxrow + 2) * sizeof(int) ); /* allocate memory */
    CalculateDotp( row, ndots, maxrow, dotp ); 

    ali->score = __DotAlign_Tube ( 
				  row, col, 
				  score, ndots,
				  roll, 
				  minrow, maxrow, 
